    ...
    for layer_idx, layer_info in enumerate(all_layers):
        ...
        for method in GRADIENT_METHODS:
            ...
            if heatmap is not None:
                ...
                # --- Enhanced Scoring Heuristic ---
                base_score = 100 - diag['border_activation_pct']
                corner_penalty = diag['corner_activation_pct'] * 2
                
                # Penalize very high entropy (diffuse blob) or very low entropy (single pixel spike)
                entropy = diag['spatial_entropy']
                entropy_penalty = 0
                if entropy > 0.8:
                    entropy_penalty = (entropy - 0.8) * 100 * 2
                elif entropy < 0.1:
                    entropy_penalty = (0.1 - entropy) * 100 * 2
                    
                # Reward distinct, focused peaks (up to 5), penalize noise (too many peaks)
                peaks = diag['peak_count']
                peak_bonus = 0
                if 1 <= peaks <= 5:
                    peak_bonus = peaks * 10
                elif 5 < peaks <= 10:
                    peak_bonus = 50 - ((peaks - 5) * 5)
                    
                score = base_score - corner_penalty - entropy_penalty + peak_bonus
